...
def adjust_lr_for_muon(lr: float, param_shape):
    A, B = param_shape[:2]
    # print("A, B:", A, B)
    adjusted_ratio = 0.5 * math.sqrt(max(A, B))
    # print("adjusted_ratio:", adjusted_ratio)
    # print("lr * adjusted_ratio:",lr, adjusted_ratio, lr * adjusted_ratio)
            ...
            if group["use_muon"]:
                ...
                for base_i in range(len(params))[::dist.get_world_size()]:
                    if base_i + dist.get_rank() < len(params):
                        ...
                        if len(state) == 0:
                            state["momentum_buffer"] = torch.zeros_like(p)
                        update = muon_update(p.grad, state["momentum_buffer"], beta=group["momentum"], ns_steps=group.get("ns_steps", 5), nesterov=True)
