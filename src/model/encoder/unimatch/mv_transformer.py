...
def batch_features_camera_parameters(
    ...
    assert (q_features[0].dim() == 4 and intrinsics[0].dim() == 3 and extrinsics[0].dim() == 3)
    assert (k_features[0].dim() == 4 and intrinsics[0].dim() == 3 and extrinsics[0].dim() == 3)
    assert (v_features[0].dim() == 4 and intrinsics[0].dim() == 3 and extrinsics[0].dim() == 3)
    ...
    num_views = len(q_features)
    if nn_matrix is not None:
        features_k = torch.stack(k_features, dim=1)  # [B, V, C, H, W]
        features_v = torch.stack(v_features, dim=1)  # [B, V, C, H, W]
    ...
    # print("nn_matrix:", nn_matrix.shape)
    # print("num_selected_views:", num_selected_views)
    for i in range(num_views):
        # features
        x_q = q_features.copy()
        x_k = k_features.copy()
        x_v = v_features.copy()
        q.append(x_q.pop(i))  # [B, C, H, W]
        x_k.pop(i)
        x_v.pop(i)
        ...
        if nn_matrix is not None:
            # select views based on the provided nn matrix
            if features_k.dim() == 5:
                c, h, w = features_k.shape[-3:]
                index = repeat(nn_matrix[:, i, 1:], "b v -> b v c h w", c=c, h=h, w=w)
            elif features_k.dim() == 4:
                hw, c = features_k.shape[-2:]
                index = repeat(nn_matrix[:, i, 1:], "b v -> b v hw c", hw=hw, c=c)

            # print(index.shape)
            # print(features_k.shape)
            k_x = torch.gather(features_k, dim=1, index=index)
            # print(k_x.shape)
            v_x = torch.gather(features_v, dim=1, index=index)
            ...
            k_x = torch.stack(x_k, dim=1)
            v_x = torch.stack(x_v, dim=1)
    ...
    # print(torch.stack(k, dim=1).shape)
    k = torch.stack(k, dim=1).view(-1, num_selected_views, c, h, w)  # [BV, V-1, C, H, W]
    v = torch.stack(v, dim=1).view(-1, num_selected_views, c, h, w)  # [BV, V-1, C, H, W]
    kv_intrinsics = torch.stack(kv_intrinsics, dim=1).view(-1, num_selected_views, 3, 3)  # [BV, V-1, 3, 3]
    kv_extrinsics = torch.stack(kv_extrinsics, dim=1).view(-1, num_selected_views, 4, 4)  # [BV, V-1, 4, 4]

    return q, q_intrinsics, q_extrinsics, k, v, kv_intrinsics, kv_extrinsics




def batch_features_camera_parameters_withD(
    q_features,
    d_features,
    k_features,
    v_features,
    ...
    assert (q_features[0].dim() == 4 and intrinsics[0].dim() == 3 and extrinsics[0].dim() == 3)
    assert (k_features[0].dim() == 4 and v_features[0].dim() == 4 and d_features[0].dim() == 4)
    assert intrinsics[0].size(-1) == intrinsics[0].size(-2) == 3
    assert extrinsics[0].size(-1) == extrinsics[0].size(-2) == 4
    ...
    num_views = len(q_features)
    if nn_matrix is not None:
        k_features = torch.stack(k_features, dim=1)  # [B, V, C, H, W]
        v_features = torch.stack(v_features, dim=1)  # [B, V, C, H, W]
        intrinsics_tensor = torch.stack(intrinsics, dim=1)  # [B, V, 3, 3]
        extrinsics_tensor = torch.stack(extrinsics, dim=1)  # [B, V, 4, 4]
    ...
    for i in range(num_views):
        ...
        if nn_matrix is not None:
            # select views based on the provided nn matrix
            if k_features.dim() == 5:
                c, h, w = k_features.shape[-3:]
                index = repeat(nn_matrix[:, i, 1:], "b v -> b v c h w", c=c, h=h, w=w)
            elif k_features.dim() == 4:
                hw, c = k_features.shape[-2:]
                index = repeat(nn_matrix[:, i, 1:], "b v -> b v hw c", hw=hw, c=c)

            k_x = torch.gather(k_features, dim=1, index=index)
            v_x = torch.gather(v_features, dim=1, index=index)
    ...
    q = torch.stack(q, dim=1).view(-1, c, h, w)  # [BV, C, H, W]
    d = torch.stack(d, dim=1).view(-1, c, h, w)  # [BV, C, H, W]
    q_intrinsics = torch.stack(q_intrinsics, dim=1).view(-1, 3, 3)  # [BV, 3, 3]
    q_extrinsics = torch.stack(q_extrinsics, dim=1).view(-1, 4, 4)  # [BV, 4, 4]
    k = torch.stack(k, dim=1).view(-1, num_selected_views, c, h, w)  # [BV, V-1, C, H, W]
    v = torch.stack(v, dim=1).view(-1, num_selected_views, c, h, w)  # [BV, V-1, C, H, W]
    ...
    return q, d, q_intrinsics, q_extrinsics, k, v, kv_intrinsics, kv_extrinsics
