...
def train(cfg_dict: DictConfig):
    if cfg_dict["mode"] == "train" and cfg_dict["train"]["eval_model_every_n_val"] > 0:
        ...
        if "re10k" in dataset_dir:
            ...
            eval_path = "assets/evaluation_index_acid.json"
        else:
    ...
    # # —— 新增：EMA 回调 ——
    # ema_decay = getattr(cfg, "ema", {}).get("decay", 0.999)
    # callbacks.append(TorchEMACallback(decay=ema_decay, cpu_offload=False))
    ...
    if cfg.mode == "train":
        ...
        if cfg.checkpointing.pretrained_monodepth is not None:
            ...
            model_wrapper.encoder.depth_predictor.depth_anything.load_state_dict(pretrained_model, strict=strict_load)
            print(
                cyan(
